...
/*            $ststem_login_errnum_key = 'system_'.md5('login_errnum_'.$user_name);
            ...
            if (!empty($condition['user_name']) && !empty($condition['password'])) {
                ...
                if (empty($admin_info)) {
                    ...
                    $platform_user_info = M('platform_user')->where(array('is_del'=>0,'admin_id'=>$admin_info['admin_id']))->find();
                    if(!$platform_user_info){
                        $this->error("加盟会员信息不存在");
                    }
                    ...
                    /*同步设置当前登录会员id值*/
                    session('platform_user_id', $platform_user_info['id']);
                    /* END */
